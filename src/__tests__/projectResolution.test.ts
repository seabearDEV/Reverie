...
beforeEach(() => {
  ...
  originalCodexProject = process.env.RVR_PROJECT;
  originalCodexNoProject = process.env.RVR_NO_PROJECT;
  delete process.env.RVR_PROJECT;
  delete process.env.RVR_NO_PROJECT;

  // Two sibling temp dirs: one bare (no .reverie), one with .codexcli/.
  // Both live under os.tmpdir(); we trust that ancestors do not contain
  // a stray .reverie/ — same assumption other tests in this suite make.
  ...
  fs.mkdirSync(path.join(storeTmpDir, '.reverie'));
...
afterEach(() => {
  setProjectRootOverride(null);
  clearProjectFileCache();
  if (originalCodexProject === undefined) delete process.env.RVR_PROJECT;
  else process.env.RVR_PROJECT = originalCodexProject;
  if (originalCodexNoProject === undefined) delete process.env.RVR_NO_PROJECT;
  else process.env.RVR_NO_PROJECT = originalCodexNoProject;
...
describe('resolveScopeForWrite — scope passthrough', () => {
  it("returns 'project' unchanged when scope is 'project' and resolution succeeds", () => {
    process.env.RVR_PROJECT = storeTmpDir;
...
describe('resolveScopeForWrite — auto / undefined resolution', () => {
  it("returns 'project' when scope is undefined and RVR_PROJECT resolves", () => {
    process.env.RVR_PROJECT = storeTmpDir;
    expect(resolveScopeForWrite(undefined)).toBe('project');
  });

  it("returns 'project' when scope is 'auto' and RVR_PROJECT resolves", () => {
    process.env.RVR_PROJECT = storeTmpDir;
...
describe('ProjectResolutionError shape', () => {
  it("carries code 'PROJECT_UNRESOLVED' and the captured diagnostic", () => {
    ...
    try {
      ...
      expect(e.diagnostic.rvrNoProject).toBe(true);
      expect(e.name).toBe('ProjectResolutionError');
    }
  });

  it('error message names the failed branch (RVR_PROJECT did not resolve)', () => {
    process.env.RVR_PROJECT = path.join(tmpDir, 'does-not-exist');
    try {
      resolveScopeForWrite(undefined);
      throw new Error('expected throw');
    } catch (err) {
      const e = err as ProjectResolutionError;
      expect(e.message).toContain('RVR_PROJECT env');
      expect(e.message).toContain('DID NOT RESOLVE TO A .reverie DIRECTORY');
      expect(e.message).toContain('reverie_init');
  ...
  it('error message names the walk-up branch when env is unset and walk exhausts', () => {
    ...
    try {
      ...
      expect(e.message).toContain('reached filesystem root without finding .reverie/');
    }
  });

  it('error message hints at unsetting RVR_NO_PROJECT when that branch fired', () => {
    process.env.RVR_NO_PROJECT = '1';
    try {
      resolveScopeForWrite(undefined);
      throw new Error('expected throw');
      ...
      const e = err as ProjectResolutionError;
      expect(e.message).toContain('unset RVR_NO_PROJECT');
    }
  });
});

describe('captureResolverDiagnostic — branch capture', () => {
  it('records rvrNoProject when RVR_NO_PROJECT is set', () => {
    process.env.RVR_NO_PROJECT = '1';
    const d = captureResolverDiagnostic();
    expect(d.rvrNoProject).toBe(true);
    expect(d.rvrProject).toBeUndefined();
    expect(d.rvrProjectFailed).toBe(false);
    expect(d.walkReachedRoot).toBe(false);
  });

  it('records rvrProject value and rvrProjectFailed when env points at a non-existent dir', () => {
    const fake = path.join(tmpDir, 'no-such-dir');
    process.env.RVR_PROJECT = fake;
    const d = captureResolverDiagnostic();
    expect(d.rvrProject).toBe(fake);
    expect(d.rvrProjectFailed).toBe(true);
  });

  it('records rvrProject value and does NOT mark failed when env resolves to a .codexcli dir', () => {
    process.env.RVR_PROJECT = storeTmpDir;
    const d = captureResolverDiagnostic();
    expect(d.rvrProject).toBe(storeTmpDir);
    expect(d.rvrProjectFailed).toBe(false);
  });
  ...
  it('records walkStoppedAtGlobalDir when walk reaches the codex global dir before any project store', () => {
    // Pin RVR_DATA_DIR (=> globalDir) and start the walk at that same dir.
    ...
    const origDataDir = process.env.RVR_DATA_DIR;
    ...
    try {
      ...
      if (origDataDir === undefined) delete process.env.RVR_DATA_DIR;
      else process.env.RVR_DATA_DIR = origDataDir;
      clearDataDirectoryCache();
    }
  });
});
...
describe('paths.ts integration', () => {
  ...
  it('findProjectFileWithDiagnostic returns the same path findProjectFile would, with a diagnostic', () => {
    ...
    expect(result.path).toBe(path.join(storeTmpDir, '.reverie'));
    expect(result.diagnostic.rvrProject).toBe(storeTmpDir);
    expect(result.diagnostic.rvrProjectFailed).toBe(false);
  });
  ...
  it('setProjectRootOverride invalidates the findProjectStoreDir cache (PR #104 review fix)', () => {
    ...
    setProjectRootOverride(storeTmpDir);
    expect(findProjectStoreDir()).toBe(path.join(storeTmpDir, '.reverie'));
